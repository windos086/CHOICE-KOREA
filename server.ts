...
async function startServer() {
  ...
  // 3-B. 실제 구글 로그인 인증 URL 요청 API (WebView 스토리 파티셔닝 타파용 웹 세이프 아키텍처)
  app.get("/api/auth/google/url", (req, res) => {
    try {
      ...
      if (!clientId || !clientSecret) {
        // 구글 환경변수 미설정 시 클라이언트 측에서 자동으로 Firebase Client SDK fallback 처리하도록 유도
        return res.status(400).json({ success: false, error: "서버에 구글 개발자 앱 환경 정보(GOOGLE_CLIENT_ID / SECRET)가 미기입 상태입니다." });
      }

      const parentOrigin = (req.query.origin as string) || "https://choicekr.co.kr";
      // 어떤 상황에서도 무조건 이 앱의 구글 콜백 경로로 리턴받도록 세팅
      const redirectUri = `${parentOrigin}/api/auth/google/callback`;

      console.log("🧩 [Google Direct Login System] Creating WebView-safe authorize URL", { parentOrigin, clientId, redirectUri });
      ...
      const params = new URLSearchParams({
        ...
        access_type: "online",
        state: redirectUri, // pass redirectUri for verification loop in state
        prompt: "select_account"
      });

      const googleRedirectLink = `https://accounts.google.com/o/oauth2/v2/auth?${params.toString()}`;
      res.json({ success: true, url: googleRedirectLink });
    } catch (err: any) {
      console.error("Error generating Google authorize URL:", err);
      res.status(500).json({ success: false, error: err.message || "구글 로그인 연동 URL 발급에 실패했습니다." });
    }
  });

  // 4-B. 실제 구글 로그인 콜백 핸들러 API (WebView 파티셔닝 해결용 토큰 교환기)
  app.get("/api/auth/google/callback", async (req, res) => {
    const { code, state, error: googleError } = req.query;

    if (googleError) {
      return res.status(400).send(`구글 로그인 실패 반환: ${googleError}`);
    }

    if (!code) {
      return res.status(400).send("구글 인증 코드(code)가 수령되지 않았습니다.");
    }

    try {
      const clientId = process.env.GOOGLE_CLIENT_ID || process.env.VITE_GOOGLE_CLIENT_ID;
      const clientSecret = process.env.GOOGLE_CLIENT_SECRET;
      const redirectUri = (state as string) || "https://choicekr.co.kr/api/auth/google/callback";

      if (!clientId || !clientSecret) {
        throw new Error("서버측 구글 클라이언트 연계 자격증명이 올바르지 않습니다.");
      }

      console.log("🔑 [Google OAuth Token Exchange] Trading code for access token:", { code, redirectUri });
      ...
      if (!tokenResponse.ok) {
        console.error("Google Token exchange failure details:", tokenData);
        throw new Error(tokenData.error_description || tokenData.error || "구글 서버로부터 연동 액세스 토큰을 받아오지 못했습니다.");
      }

      const accessToken = tokenData.access_token;
      console.log("🟢 [Google Access Token Acquired]");

      // Google User Profile Retrieve
      ...
      if (!userResponse.ok) {
        console.error("Google Profile Retrieve failed:", userData);
        throw new Error(userData.error_description || "구글 프로필 조회를 실행하지 못했습니다.");
      }

      console.log("👤 [Google user profile payload]:", userData);

      const id = userData.sub || Math.random().toString(36).substring(2, 11);
      const email = userData.email || `${id}@googleuser.com`;
      const nickname = userData.name || userData.given_name || email.split("@")[0] || "구글테스터";
      ...
      return res.send(`
        <html>
          ...
          <body style="font-family: -apple-system, sans-serif; text-align: center; padding-top: 100px; background-color: #f9fafb;">
            <div style="max-width: 400px; margin: 0 auto; background: white; padding: 40px 30px; border-radius: 16px; box-shadow: 0 10px 15px -3px rgba(0,0,0,0.1);">
              <div style="background: #4285f4; width: 64px; height: 64px; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 20px; color: white; font-size: 32px; font-weight: bold; line-height: 1;">
                G
              </div>
              <h2 style="color: #111827; margin-bottom: 8px; font-weight: 800;">Google 로그인 완료</h2>
              <p style="color: #6b7280; font-size: 14px; margin-bottom: 24px;">반갑습니다, <strong>${nickname}</strong>님.<br>구글 본인인증 계정이 안전하게 연합되었습니다.</p>
              <p style="font-size: 12px; color: #9ca3af;">잠시 후 이 화면이 자동으로 정리 및 연동됩니다.</p>
            </div>
            <script>
              try {
                if (window.opener) {
                  ...
                  console.log("Sending Google OAuth success payload to opener:", payload);
                  window.opener.postMessage(payload, "*");
                  window.close();
                } else {
                  console.warn("Opener window not found (Direct Redirect Model). Redirecting to home with credentials.");
                  const qParams = 'google_login_success=true' +
                    '&email=' + encodeURIComponent(${JSON.stringify(email)}) +
                    '&nickname=' + encodeURIComponent(${JSON.stringify(nickname)}) +
                    '&id=' + encodeURIComponent(${JSON.stringify(id)});
                  window.location.href = '/?' + qParams;
                ...
                console.error("Error communicating with opener:", err);
                alert("로그인을 홈페이지에 반영하는 중에 오류가 발생했습니다: " + err.message);
              }
            </script>
          </body>
        </html>
      ...
      return res.status(500).send(`구글서버 연동 토큰 교환 도중 예외가 발생했습니다: ${err.message || err}`);
    }
  });
